 ...
 *
 * @method     color findOneById(int $id) Return the first color filtered by the id column
...
abstract class BasecolorQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'propel', $modelName = 'color', $modelAlias = null)
	{
	 ...
	 *
	 * @return    color|array|mixed the result, formatted by the current formatter
	 */
	...
	protected function findPkSimple($key, $con)
	{
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new color();
			$obj->hydrate($row);
			colorPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
